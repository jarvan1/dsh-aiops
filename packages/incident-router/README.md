...
At runtime the router registers `aiops-routing` settings version 1 over the composition values. Portal changes to noise exclusions, severity normalization/default, model budgets, cooldown, queue, concurrency, and reservations are validated, revision-fenced, applied live, and persisted as a secret-free `routing_policy_audit` record. `dryRun()` evaluates normalized labels without writing queue, audit, or Session state. Pre-F.2 profiles containing `alertnameAllowlist` still start with safe general-routing defaults and a deprecation warning; replace the obsolete field with explicit `routingPolicy` after upgrade.
